...
public function __construct()
 ...
 // عرض القوائم والتفاصيل والفواتير
    ...
    // جلب تفاصيل كود المسوّق (للأوتوكومبليت)
    $this->middleware('can:commissions.view')
         ->only(['details']);
    }
    /**
...
public function index(Request $request)
{
    ...
    $mainTab = $request->get('mainTab', 'all');    // all|marketer|employee
    $subTab  = $request->get('subTab', 'all');     // all|delivered|pending

    // 1) Base query (بدون subTab)
    ...
    // فلاتر البحث/التاريخ (نفس الموجودة عندك حالياً):
    ...
    // 3) نطبّق subTab على الاستعلام اللي هنرجّعه للواجهة
    $query = clone $base;
    if ($subTab === 'delivered')  $query->where('received', 1);
    if ($subTab === 'pending')    $query->where('received', 0);

    // تحديد عدد العناصر في كل صفحة من الريكوست أو الديفولت 10
...
$today = now()->translatedFormat('l');     // اليوم بالعربي
$date  = now()->translatedFormat('d F Y'); // التاريخ بالعربي
...
$callback = function () use ($rows, $dynamicCols, $type, $subTab, $total_commission, $total_invoice, $total_visitors, $total_dishes, $today, $date) {
    ...
    echo '</td>';
    echo '</tr>';
    // 3 empty rows under the image and before today/date
    echo '<tr><td colspan="3" style="height:18px;"></td></tr>';
    echo '<tr><td colspan="3" style="height:18px;"></td></tr>';
    echo '<tr><td colspan="3" style="height:18px;"></td></tr>';
    // اليوم والتاريخ في صف جديد تحت الصورة
    echo '<tr>';
...
echo '<div style="height:40px;"></div>';
    ...
    foreach ($dynamicCols as $key => $label) {
        // بعد عمود الاسم، أضف عمود الهاتف
        if ($key === 'person') {
    ...
    foreach ($rows as $r) {
        ...
        $personName  = $r->marketer->name ?? $r->marketingEmployee->name ?? '—';
        $personPhone = $r->marketer->phone ?? $r->marketingEmployee->phone ?? '—';
        // Force phone to be treated as text in Excel (avoid scientific notation)
        if (is_numeric($personPhone) && strpos($personPhone, '+') === 0) {
            // If phone starts with +, wrap in ="..." for Excel
            $personPhone = '="' . $personPhone . '"';
        } elseif (is_numeric($personPhone)) {
            // For pure numbers, also wrap in ="..." to force text
            $personPhone = '="' . $personPhone . '"';
        }
        // الاسم
        echo '<td>'.e($personName).'</td>';
        // رقم الهاتف (عمود منفصل)
...
/**
 * Check if promo code exists for marketer or employee.
 */
...
/**
 * Check if delivery code is valid for a commission.
 */

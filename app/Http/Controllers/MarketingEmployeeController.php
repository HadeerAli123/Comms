...
class MarketingEmployeeController extends BaseController

{
    use AuthorizesRequests, ValidatesRequests;
public function __construct()
    {
  $this->middleware('permission:marketing-employees.view')
        ->only(['index','show','details']);

    $this->middleware('permission:marketing-employees.create')
        ->only(['create','store']);

    $this->middleware('permission:marketing-employees.update')
        ->only(['edit','update']);

    $this->middleware('permission:marketing-employees.delete')
        ->only(['destroy']);
    }
...
public function update(Request $request, User $marketing_employee)
{
    $data = $request->validate([
        ...
        'name'     => ['required','string','max:255'],
        'email'    => ['required','email','unique:users,email,'.$marketing_employee->id],
        'phone'    => ['required', 'string', 'max:20'],
        'country_code' => ['required', 'string', 'max:6'],
        'password' => ['nullable','string','min:8','confirmed'],
        'percentage' => ['required', 'numeric', 'min:0', 'max:100'],
        'table_commission' => ['required', 'numeric', 'min:0'],
    ], [
        'percentage.required' => 'النسبة مطلوبة.',
        'percentage.numeric'  => 'النسبة يجب أن تكون رقمية.',
        'percentage.min'      => 'النسبة يجب ألا تقل عن 0.',
        ...
        'table_commission.required' => 'مبلغ عمولة الطاولة مطلوب.',
        'table_commission.numeric' => 'مبلغ عمولة الطاولة يجب أن يكون رقمياً.',
    ...
    $update = [
        ...
        'phone'    => $fullPhone,
        'prec'     => $data['percentage'],
        'table_commission' => $data['table_commission'],
    ];
